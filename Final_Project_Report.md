...
| **Phase 6: Deployment** | Week 11 | Dockerization, AWS Setup, CI/CD Pipeline | ✅ Completed |
| **Phase 8: Optimization** | Week 12 | Hybrid WASM Execution, Security Hardening | ✅ Completed |
...
| **Code Execution Security** | Medium | Critical | Use Client-Side execution (WASM) + Docker containers. |
...
## 7. Advanced Optimization (Phase 8 Implementation)

### 7.1 Hybrid Execution Architecture
The project successfully implemented a "Hybrid Architecture" to balance performance and cost.
*   **Client-Side (WASM/Native)**: Python (via Pyodide) and JavaScript execute directly in the user's browser. This results in instant feedback and **$0 server cost** for 80% of execution requests.
*   **Server-Side (Docker)**: C++ and Java, which require heavy compilation, are handled by isolated Docker containers on the backend.

### 7.2 Security Hardening
To prevent abuse, the following security layers were integrated:
1.  **Rate Limiting**: Global limit of 100 requests/15min to prevent DDOS. Strict 5 requests/min limit on the expensive `/execute` endpoint.
2.  **Helmet.js**: Implemented secure HTTP headers to mitigate XSS and other web vulnerabilities.
3.  **Docker Isolation**: User code execution on the server is sandboxed within Docker containers to prevent file system access or RCE attacks.

### 7.3 Database Tuning
*   **Connection Pooling**: Configured `maxPoolSize: 10` to manage MongoDB connections efficiently under load.
*   **Indexing**: Added composite indexes to `Snippet` (`userId`, `isShared`) and `Discussion` (`lessonId`) collections, reducing query time from O(n) to O(log n).
